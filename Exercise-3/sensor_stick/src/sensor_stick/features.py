...
def compute_color_histograms(cloud, using_hsv=False):
    ...
    #print(len(ch1_hist),len(ch2_hist),len(ch3_hist))
    # TODO: Concatenate and normalize the histograms
    hist_features = np.concatenate((ch1_hist[0], ch2_hist[0], ch3_hist[0])).astype(np.float64)
    # Normalize the result
    if hist_features.sum() > 0.0:
        normed_features = hist_features/hist_features.sum()
    #print("c",len(hist_features))
...
def compute_normal_histograms(normal_cloud):
    ...
    nbins = 32
    bins_range = (0,256)
    #normed_features = np.zeros((3*nbins))
    #normed_features = normed_features.astype(np.float64)
    ...
    if hist_features.sum() > 0.0:
        normed_features = hist_features / hist_features.sum()
    #print("n", len(hist_features), hist_features.shape, hist_features.sum(), len(normed_features), np.sum(normed_features))
    #normed_features = hist_features
    # Generate random features for demo mode.  
    # Replace normed_features with your feature vector
    #normed_features = np.random.random(96)
